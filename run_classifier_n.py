...
DEFINE_bool("use_tpu", False, "Whether to use TPU or GPU/CPU.")
...
DEFINE_string("master", None, "[Optional] TensorFlow master URL.")

DEFINE_integer(
    "num_tpu_cores", 8,
    "Only used if `use_tpu` is True. Total number of TPU cores to use.")

DEFINE_string("lr_decay", 'poly', "[Optional] TensorFlow lr decay method.")
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      if i == 0:
        continue
      guid = "%s-%s" % (set_type, i)
      text_a = tokenization.convert_to_unicode(self._clean_text(line[1]))
      if set_type == "test":
        label = "0"
      else:
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      train_op = optimization.create_optimizer(
          ...
          use_tpu, lr_decay=FLAGS.lr_decay)
